...
public interface FineRepository extends JpaRepository<FineEntity, Long> {
    List<FineEntity> findAllByAppTeamIdOrderByNameAsc(Long appTeamId);
    ...
    Optional<FineEntity> findFirstByCodeAndAppTeamIdAndInactiveFalse(String code, Long appTeamId);

    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("SELECT fine FROM fine fine WHERE fine.id = :fineId")
    Optional<FineEntity> findByIdForUpdate(@Param("fineId") Long fineId);
}
