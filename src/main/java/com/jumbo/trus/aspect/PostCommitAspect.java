...
public class PostCommitAspect {
    ...
    public void executePostCommitTask(JoinPoint joinPoint, Object result) {
        ...
        Runnable task = () -> {
            achievementService.updateAllPlayerAchievements(appTeam, achievementType);
            ...
            TransactionSynchronizationManager.registerSynchronization(
                    new TransactionSynchronization() {
                        ...
                        public void afterCommit() {
                            log.info("Spouštím asynchronní úkol po transakci pro appTeamId={}", appTeam.getId());
                            taskExecutor.execute(task);
                        }
                    }
            );
        } else {
            log.warn("Transakce není aktivní, spouštím ihned pro appTeamId={}", appTeam.getId());
            taskExecutor.execute(task);
        }
    }
